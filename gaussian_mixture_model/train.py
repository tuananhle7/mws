...
def train_mws(
    ...
    optimizer = torch.optim.Adam(
        itertools.chain(generative_model.parameters(), inference_network.parameters())
    )
    (
        theta_losses,
        phi_losses,
    ...
    for iteration in range(num_iterations):
        ...
        for single_obs in obs:
            ...
            if (single_obs_key not in memory) or len(memory[single_obs_key]) == 0:
                # batch shape [1] and event shape [num_data]
                latent_dist = inference_network.get_latent_dist(single_obs.unsqueeze(0))

                # HACK
                while True:
                    # [memory_size, num_data]
                    latent = inference_network.sample_from_latent_dist(
                        latent_dist, memory_size
                    ).squeeze(1)
                    # list of M \in {1, ..., memory_size} elements
                    # could be less than memory_size because
                    # sampled elements can be duplicate
                    memory[single_obs_key] = list(set([tuple(x.tolist()) for x in latent]))
            ...
            latent = inference_network.sample_from_latent_dist(latent_dist, 1).view(-1)
            ...
            memoized_latent_plus_current_latent_tensor = torch.tensor(
                list(memoized_latent_plus_current_latent), device=single_obs.device
            ).unsqueeze(1)
            # [memory_size + 1]
            log_p_tensor = generative_model.get_log_prob(
                memoized_latent_plus_current_latent_tensor, single_obs.unsqueeze(0)
            ).squeeze(-1)
            ...
            log_p = {
                mem_latent: lp
                for mem_latent, lp in zip(memoized_latent_plus_current_latent, log_p_tensor)
            }

            # update memory.
            # {float: list of ints}
            memory[single_obs_key] = sorted(memoized_latent_plus_current_latent, key=log_p.get)[
                -memory_size:
            ]
            ...
            if reweighted:
                ...
                inference_network_log_prob = inference_network.get_log_prob_from_latent_dist(
                    latent_dist, memory_latent[:, None, :]
                ).squeeze(
                    -1
                )  # [memory_size]

                theta_loss += -torch.sum(
                    memory_log_weight * memory_weight_normalized.detach()
                ) / len(obs)
                phi_loss += -torch.sum(
                    inference_network_log_prob * memory_weight_normalized.detach()
                ) / len(obs)
            else:
                remembered_latent_id_dist = torch.distributions.Categorical(
                    logits=torch.tensor(list(map(log_p.get, memory[single_obs_key])))
                )
                ...
                # []
                theta_loss += -(
                    log_p.get(remembered_latent) - remembered_latent_id_log_prob.detach()
                ) / len(obs)
                # []
                phi_loss += -inference_network.get_log_prob_from_latent_dist(
                    latent_dist, remembered_latent_tensor
                ).view(()) / len(obs)
        ...
        cluster_cov_distances.append(
            torch.norm(true_cluster_cov - generative_model.get_cluster_cov()).item()
        )
        ...
        if iteration % 100 == 0:  # test every 100 iterations
            (
                ...
                true_generative_model, generative_model, inference_network, None, test_data_loader
            )
            ...
            (
                ...
                true_generative_model, generative_model, inference_network, memory, data_loader
            )
...
def train_rws(
    ...
    for iteration in range(num_iterations):
        ...
        wake_theta_loss, elbo = losses.get_wake_theta_loss_from_log_weight(log_weight)
...
def train_vimco(
    ...
    for iteration in range(num_iterations):
        ...
        # loss
        optimizer.zero_grad()
        loss, elbo = losses.get_vimco_loss(generative_model, inference_network, obs, num_particles)
        ...
        util.print_with_time("it. {} | theta loss = {:.2f}".format(iteration, loss))
