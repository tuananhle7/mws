...
def get_path_base_from_args(args):
    return f"{args.algorithm}_{args.num_particles}_{args.seed}"
...
def init(num_data, num_dim, true_cluster_cov, device):
    ...
    generative_model = models.GenerativeModel(num_data, prior_loc, prior_cov, device).to(device)
    inference_network = models.InferenceNetwork(num_data, num_dim).to(device)
    true_generative_model = models.GenerativeModel(
        num_data, prior_loc, prior_cov, device, true_cluster_cov
    ).to(device)
...
def load_checkpoint(path, device):
    ...
    generative_model, inference_network, _ = init(
        checkpoint["num_data"], checkpoint["num_dim"], true_cluster_cov, device
    )
...
def generate_and_save_data():
    ...
    _, _, true_generative_model = init(num_data, num_dim, true_cluster_cov, device)
    train_data = true_generative_model.sample_obs(num_train)
    test_data = true_generative_model.sample_obs(num_test)
    torch.save({"train_data": train_data, "test_data": test_data}, "data.pt")
